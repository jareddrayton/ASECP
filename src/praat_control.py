...
def get_formants(file_path):
    """
    Read a formant table generated by praat and average 
    """
    ...
    win_size = 0.5
    win_pos = 0.5
    win_len = round(len(data) * win_size)
    window_start = round(win_pos * (len(data))) - round(win_len / 2)
    window_end = round(win_pos * (len(data))) + round(win_len / 2)

    data = data[window_start:window_end+1]
    ...
    for formant in formants:
        average_formants.append(np.mean([x for x in formant if x is not None]))
...
def write_formant_table(file_path, name, sound_type='Individual'):
    ...
    formant_table_config(praat_script, audio_file, formant_table)
...
def formant_table_config(praat_script, audio_file, formant_table):

    with open(praat_script, 'w') as f:
        f.write('Read from file: "{}"\n'.format(audio_file))
        f.write('nowarn To Formant (sl): 0, 5, 5000, 0.025, 50\n')

...
def linear_ranking(population, keys):
    ...
    # Unpack the list of tuples into seperate tuples
    ranked, _ = zip(*keys_fitness)
    # Set the constant term to equal to the population size
    C = len(population)
...
def uniform_crossover(population, keys):
    ...
    for i, name in enumerate(keys):
        tt = [random.randint(0, 1)
              for x in range(len(population[keys[i]].values))]
        hold.append([population[mothers[i]].values[j] if tt[j] is True
    ...
    for name in keys:
        for i in range(len(population[name].values)):
            ...
            if mutation_rate >= random.random():
                ...
                population[name].values[i] = round(
                    population[name].values[i] + random.gauss(0, mutation_standard_dev * scale_factor), 2)
